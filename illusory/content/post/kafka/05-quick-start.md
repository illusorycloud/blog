---
title: "kafka(Go)教程(五)---Proudcer-Consumer API 基本使用"
description: "Kafka Go sarama 客户端同步异步生产者,独立消费者和消费者组的基本使用"
date: 2021-08-13 21:00:00
draft: false
categories: ["Kafka"]
tags: ["Kafka"]
---

本文主要讲解其中的 Producer API 和 Consumer API 在 Go Client sarama 中的使基本使用。
...
> 其中 Stream API 已经明确表示不会支持，Connect 还不确定。
...
消费者组的好出在于并发消费，Kafka 把分发逻辑已经实现了，我们只需要启动多个消费者即可。

> 如果只有一个消费者，我们需要手动获取消息后分发给多个消费者，需要多写一段代码，而且 Offset 维护还比较麻烦。
